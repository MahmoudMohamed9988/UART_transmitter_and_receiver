vlog pkg.sv
vlog -cover bcs UART_transmitter.v 
...
file mkdir $regressionoutputDir
 ...
 set UART_testcases [list UART_master_test]


foreach testcases $UART_testcases {
  #run tests n times
 set tests 3
 ...
 for { set n 0 } { $n < $tests } { incr n } {
  #build unique names for each simulation
	...
	vsim -sv_seed $n -onfinish stop -coverage +UVM_TESTNAME=UART_master_test tb_top
